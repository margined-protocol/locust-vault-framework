...
fn test_add_to_queue() {
    ...
    // Add the first entry
    add_to_queue(&mut storage, user.clone(), Uint128::new(100), 1672531200).unwrap();
    ...
    add_to_queue(&mut storage, user.clone(), Uint128::new(50), 1672531300).unwrap();
    let redemption = redemptions().load(&storage, user.as_str()).unwrap();
...
fn test_remove_from_queue() {
    ...
    let amount = remove_from_queue(&mut storage, user.clone()).unwrap();
...
fn test_remove_nonexistent_user() {
    ...
    let result = remove_from_queue(&mut storage, user.clone());
