...
fn test_process_redeem() {
    ...
    env.app.increase_time(ONE_WEEK_IN_SECONDS);

    env.process_deposits(&wasm, &vault_addr, None, None, &env.controller)
        .unwrap();
    env.process_redemptions(&wasm, &vault_addr, None, None, &env.controller)
        .unwrap();

    let state_next = env.query_state_fund(&wasm, &vault_addr).unwrap();
    ...
    assert!(state.total_pending_deposits.is_zero());
    assert!(state.total_pending_redemptions.is_zero());
    assert!(!state.deposits_processed);
    assert!(!state.redemptions_processed);
    assert_eq!(
        state.next_subscription_process,
        state_next
            .next_subscription_process
            .plus_seconds(ONE_WEEK_IN_SECONDS)
    );
    ...
    let queue = env
        .query_redemption_queue(&wasm, &vault_addr, None, None)
        .unwrap();
...
#[test]
fn test_process_redeem_alt_denom() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);

    let mut msg = env.default_fund_instantiation_msg();

    msg.alt_denom = Some(AltDenom {
        denom: QUOTE_DENOM.to_string(),
        pool_id: env.default_pool.id.to_string(),
    });
    ...
    assert_approx_eq!(contract_base_before, contract_base_after, "1");
    assert_approx_eq!(contract_quote_before, contract_quote_after, "1");
...
#[test]
fn test_process_redeem_multiple_users() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);
...
#[test]
fn test_process_redeem_both_users() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);
...
#[test]
fn test_process_redeem_end_to_end_with_repayment() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);
    ...
    // Controller withdraws
    {
        ...
        env.withdraw(&wasm, &vault_addr, vec![withdraw_amount], &env.controller)
            .unwrap();
    }
    ...
    let expected_state = StateResponse {
        ...
        next_subscription_process: block_time
            .plus_seconds(config.subscription_duration)
            .plus_seconds(config.subscription_duration),
        deposits_processed: false,
        redemptions_processed: false,
        total_pending_deposits: Uint128::zero(),
        total_pending_redemptions: Uint128::zero(),
    };
    ...
    // Controller repays - with profit
    {
        ...
        let repay_amount = coin(200_000_000, BASE_DENOM);
        env.repay_fund(&wasm, &vault_addr, &[repay_amount], &env.controller)
            .unwrap();
    }
    ...
    // Trader redeems
    {
        ...
        let expected_state = StateResponse {
            ...
            redemptions_processed: false,
            total_pending_deposits: Uint128::zero(),
            total_pending_redemptions: redeem.amount,
        };
...
#[test]
fn test_process_redeem_end_to_end_with_partial_repayment() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);
    ...
    {
        let repay_amount = coin(100_000_000, BASE_DENOM);
        env.repay_fund(&wasm, &vault_addr, &[repay_amount], &env.controller)
            .unwrap();
    }
...
#[test]
fn test_process_redeem_end_to_end_with_repayment_alt_denom() {
    let env = TestEnv::new();
    let wasm = Wasm::new(&env.app);
    ...
    let expected_state = StateResponse {
        is_open: true,
        is_paused: false,
        total_staked_tokens: Uint128::new(339_999_999),
    ...
    // Controller repays - with profit
    {
        ...
        env.repay_fund(
            &wasm,
            &vault_addr,
            &[repay_amount_base, repay_amount_quote],
            &env.controller,
        )
        .unwrap();
    }
