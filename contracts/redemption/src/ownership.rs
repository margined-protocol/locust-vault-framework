...
use cosmwasm_std::{
    attr, ensure, ensure_eq, Deps, DepsMut, Env, Event, MessageInfo, Response, StdError, StdResult,
};
use cw_controllers::Admin;
use cw_storage_plus::Item;
use interface::redemption::OwnerProposal;
...
pub fn handle_ownership_proposal(
    ...
    let proposed_owner = deps.api.addr_validate(proposed_owner.as_str())?;
        ...
        ContractError::Std(StdError::generic_err("Invalid ownership proposal"))
    );
...
pub fn handle_claim_ownership(
    ...
    let p = proposal
        .load(deps.storage)
        .map_err(|_| ContractError::Std(StdError::generic_err("Proposal not found")))?;
